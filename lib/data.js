...
var newestUrl = 'http://88.196.48.87:8888/solr/select?q=*%3A*&rows=10&wt=json&indent=true';
...
function organize(arrayOfPosts){
	...
	for(var x = 0; x < arrayOfPosts.length; x++){
		...
		if(typeof arrayOfPosts[x].category !== 'undefined'){
			obj.img = UrlExists(arrayOfPosts[x].category);
		} else {
			obj.img = 'Not specified';
		}
...
function findIMG(array){
	...
	for (var x = 0; x < cats.length; x++){
		var imgUrl = 'http://a.fsdn.com/sd/topics/' + cats[x] + '_64.png"'
		var checkUrl = UrlExists(imgUrl);
	}
...
function cleanDescription(descr){
	...
	// cuts the content of description leaving 25 words
	var cut1 = clean2.split(' ');	
	var cut2 = cut1.splice(0, 25);	
	var join1 = cut2.join(' ');
